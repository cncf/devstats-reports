with events as (
  ...
  where
    ...
    and actor_id not in (select actor_id from gha_events where type in ('PullRequestEvent', 'IssuesEvent', 'PullRequestReviewCommentEvent', 'PullRequestReviewEvent') and created_at < now() - '{{ago}}'::interval)
), known_events as (
  ...
  where
    ...
    and e.actor_id not in (select actor_id from gha_events where type in ('PullRequestEvent', 'IssuesEvent', 'PullRequestReviewCommentEvent', 'PullRequestReviewEvent') and created_at < now() - '{{ago}}'::interval)
