select
  ...
  (sub.company_contributors * 100.0) / sub.{{type}}_contributors as percent_contributors
from (
  select count(distinct e.dup_actor_login) filter (where af.company_name = '{{company}}') as company_contributors,
    count(distinct e.dup_actor_login) filter (where af.company_name is not null) as known_contributors,
