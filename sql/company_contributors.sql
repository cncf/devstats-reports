...
from (
  select count(distinct e.actor_id) filter (where af.{{company_name}} = '{{company}}') as company_contributors,
    count(distinct e.actor_id) filter (where af.{{company_name}} is not null) as known_distinct_contributors,
    count(distinct e.actor_id) as all_distinct_contributors,
    count(e.actor_id) filter (where af.{{company_name}} is not null) as known_contributors,
    count(e.actor_id) as all_contributors
  from
    gha_events e
  left join
